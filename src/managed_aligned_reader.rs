...
use crate::global_file_manager::{GlobalFileManager, pread_fd};
use crate::io_stats::IoStatsTracker;

const DEFAULT_BUF_SIZE: usize = 64 * 1024; // 64KB
const ALIGNMENT: usize = 4096;

/// AlignedReader that uses GlobalFileManager for file operations
pub struct ManagedAlignedReader {
...
impl ManagedAlignedReader {
    /// Create a new ManagedAlignedReader
    pub fn new(
        file_manager: Arc<GlobalFileManager>,
        path: impl AsRef<Path>,
    ) -> io::Result<Self> {
        Self::with_start_position(
            file_manager,
            path,
            0, // Start at beginning
            DEFAULT_BUF_SIZE,
            None, // No I/O tracker by default
        )
    }
    ...
    pub fn new_from_raw_fd(
        ...
        let aligned_buffer_size = (buffer_size + ALIGNMENT - 1) & !(ALIGNMENT - 1);

        // Calculate aligned position and skip bytes
        let aligned_pos = (start_byte / ALIGNMENT as u64) * ALIGNMENT as u64;
        let skip_bytes = (start_byte - aligned_pos) as usize;
        
        let mut reader = Self {
            fd,
            buffer: AlignedBuffer::new(aligned_buffer_size, ALIGNMENT)?,
    ...
    /// Create a new ManagedAlignedReader with start position, buffer size and I/O tracker
    pub fn with_start_position(
        ...
        // Get the raw file descriptor from the file manager
        let fd = file_manager.get_fd(path.as_ref())?;
    ...
    fn refill_buffer(&mut self) -> io::Result<usize> {
        ...
        // Debug: println!("Bytes read: {}, buffer size: {}, file offset: {}", bytes_read, self.buffer_size, self.file_offset);
        ...
        if bytes_read == 0 {
            ...
            if bytes_read < self.buffer_size {
                ...
                let next_aligned = ((self.file_offset + bytes_read as u64 + self.alignment as u64 - 1) 
                                   / self.alignment as u64) * self.alignment as u64;
    ...
    pub fn seek(&mut self, pos: u64) -> io::Result<()> {
        ...
        let aligned_pos = pos & !(self.alignment as u64 - 1);
        let offset_within_block = (pos - aligned_pos) as usize;

        // If seeking within the current buffer, just adjust offset
        if self.buffer_valid_len > 0 && self.file_offset >= self.buffer_size as u64 {
            let current_buffer_start = self.file_offset - self.buffer_size as u64;
            let current_buffer_end = current_buffer_start + self.buffer_valid_len as u64;
            
            if pos >= current_buffer_start && pos < current_buffer_end {
                // We can seek within the current buffer
                let new_offset = (pos - current_buffer_start) as usize;
        ...
        if offset_within_block > 0 {
            ...
            self.refill_buffer()?;
            self.buffer_offset = offset_within_block.min(self.buffer_valid_len);
        }
...
impl Read for ManagedAlignedReader {
...
impl Seek for ManagedAlignedReader {
    fn seek(&mut self, pos: SeekFrom) -> io::Result<u64> {
        let new_pos = match pos {
            SeekFrom::Start(pos) => pos,
            SeekFrom::End(_offset) => {
                // TODO: For SeekFrom::End, we would need to track file sizes
                // For now, return an error
                return Err(io::Error::new(
                    io::ErrorKind::Unsupported,
                    "SeekFrom::End not supported - file size tracking not implemented"
                ));
            }
...
pub struct ManagedAlignedChunkReader {
    /// Raw file descriptor for direct operations
    fd: RawFd,
    file_size: u64,
    buffer_size: usize,
...
impl ManagedAlignedChunkReader {
    pub fn new(
        file_manager: Arc<GlobalFileManager>,
        path: impl AsRef<Path>,
        buffer_size: usize,
    ) -> Result<Self, String> {
        Self::new_with_tracker(file_manager, path, buffer_size, None)
    }
    ...
    pub fn new_with_tracker(
        ...
        let fd = file_manager.get_fd(path.as_ref())
            .map_err(|e| format!("Failed to get file descriptor: {}", e))?;
...
impl Length for ManagedAlignedChunkReader {
    fn len(&self) -> u64 {
        self.file_size
    }
}

impl ChunkReader for ManagedAlignedChunkReader {
    type T = ManagedAlignedReader;

    fn get_read(&self, start: u64) -> ParquetResult<Self::T> {
        // Create a new reader starting at the specified position
        let reader = ManagedAlignedReader::new_from_raw_fd(
            self.fd,
            start,
            self.buffer_size,
...
mod tests {
    ...
    fn create_test_file(dir: &Path, name: &str, size: usize) -> io::Result<(PathBuf, Vec<u8>)> {
    ...
    fn create_test_file_with_data(dir: &Path, name: &str, data: &[u8]) -> io::Result<PathBuf> {
        let path = dir.join(name);
        let mut file = File::create(&path)?;
        file.write_all(data)?;
        file.sync_all()?;
    ...
    fn test_basic_read() {
        ...
        let file_manager = Arc::new(GlobalFileManager::new(ALIGNMENT));
        let test_size = 8192;
        let (file_path, test_data) = create_test_file(temp_dir.path(), "test.dat", test_size).unwrap();

        let mut reader = ManagedAlignedReader::new(file_manager, &file_path).unwrap();
    ...
    fn test_small_reads() {
        let temp_dir = TempDir::new().unwrap();
        let file_manager = Arc::new(GlobalFileManager::new(ALIGNMENT));
        let (file_path, test_data) = create_test_file(temp_dir.path(), "test.dat", 8192).unwrap();
    ...
    fn test_position_tracking() {
        let temp_dir = TempDir::new().unwrap();
        let file_manager = Arc::new(GlobalFileManager::new(ALIGNMENT));
        create_test_file(temp_dir.path(), "test.dat", 8192).unwrap();
    ...
    fn test_multiple_readers_same_file() {
        ...
        let mut reader1 = ManagedAlignedReader::new(Arc::clone(&file_manager), &file_path).unwrap();
        let mut reader2 = ManagedAlignedReader::new(Arc::clone(&file_manager), &file_path).unwrap();
    ...
    fn test_eof_handling() {
        let temp_dir = TempDir::new().unwrap();
        let file_manager = Arc::new(GlobalFileManager::new(ALIGNMENT));
        create_test_file(temp_dir.path(), "test.dat", 1024).unwrap();
        let file_path = temp_dir.path().join("test.dat");
        let mut reader = ManagedAlignedReader::new(file_manager, &file_path).unwrap();
    ...
    fn test_multiple_seeks_and_reads() {
        ...
        for (seek_pos, read_len) in operations {
            ...
            reader.read_exact(&mut buf).unwrap();
            assert_eq!(buf, &test_data[seek_pos as usize..(seek_pos as usize + read_len)]);
        }
    }
    ...
    fn test_skip_to_newline() {
        ...
        let file_path = create_test_file_with_data(temp_dir.path(), "test.dat", test_data).unwrap();
        
        // Test skipping from middle of first line
        let mut reader = ManagedAlignedReader::new(Arc::clone(&file_manager), &file_path).unwrap();
    ...
    fn test_seek_trait() {
        ...
        let pos = <ManagedAlignedReader as Seek>::seek(&mut reader, SeekFrom::Start(1000)).unwrap();
        assert_eq!(pos, 1000);
        assert_eq!(reader.position(), 1000);

        // Test SeekFrom::Current with positive offset
        let pos = <ManagedAlignedReader as Seek>::seek(&mut reader, SeekFrom::Current(500)).unwrap();
        assert_eq!(pos, 1500);
        assert_eq!(reader.position(), 1500);

        // Test SeekFrom::Current with negative offset
        let pos = <ManagedAlignedReader as Seek>::seek(&mut reader, SeekFrom::Current(-200)).unwrap();
        assert_eq!(pos, 1300);
        assert_eq!(reader.position(), 1300);

        // Test SeekFrom::End with negative offset
        let pos = <ManagedAlignedReader as Seek>::seek(&mut reader, SeekFrom::End(-100)).unwrap();
    ...
    fn test_io_tracker() {
        ...
        create_test_file(temp_dir.path(), "test.dat", test_size).unwrap();
        let file_path = temp_dir.path().join("test.dat");
        let tracker = IoStatsTracker::new();
        let mut reader = ManagedAlignedReader::new_with_tracker(
            file_manager,
            &file_path,
            Some(tracker.clone())
        ).unwrap();
    ...
    fn test_chunk_reader_interface() {
        ...
        let file_path = create_test_file_with_data(temp_dir.path(), "test.dat", &test_data).unwrap();
        let chunk_reader = ManagedAlignedChunkReader::new(
    ...
    fn test_read_line_basic() {
        ...
        let file_path = create_test_file_with_data(temp_dir.path(), "test_lines.dat", content.as_bytes()).unwrap();
        let mut reader = ManagedAlignedReader::new(file_manager, &file_path).unwrap();
    ...
    fn test_read_line_no_final_newline() {
        ...
        let file_path = create_test_file_with_data(temp_dir.path(), "test_no_newline.dat", content.as_bytes()).unwrap();
        let mut reader = ManagedAlignedReader::new(file_manager, &file_path).unwrap();
    ...
    fn test_read_line_long_lines() {
        ...
        let content = format!("{}\nshort\n", long_line);
        let file_path = create_test_file_with_data(temp_dir.path(), "test_long_lines.dat", content.as_bytes()).unwrap();
        let mut reader = ManagedAlignedReader::new(file_manager, &file_path).unwrap();
    ...
    fn test_read_line_empty_lines() {
        ...
        let content = "\n\nline\n\n";
        let file_path = create_test_file_with_data(temp_dir.path(), "test_empty_lines.dat", content.as_bytes()).unwrap();
        let mut reader = ManagedAlignedReader::new(file_manager, &file_path).unwrap();
    ...
    fn test_read_line_crlf() {
        ...
        let content = "Line 1\r\nLine 2\r\n";
        let file_path = create_test_file_with_data(temp_dir.path(), "test_crlf.dat", content.as_bytes()).unwrap();
        let mut reader = ManagedAlignedReader::new(file_manager, &file_path).unwrap();
    ...
    fn test_direct_io_alignment_edge_cases() {
        ...
        let test_cases = vec![
            (ALIGNMENT - 1, "one byte less than alignment"),
            (ALIGNMENT, "exactly aligned"),
            (ALIGNMENT + 1, "one byte more than alignment"),
            (ALIGNMENT * 2 - 1, "one byte less than 2x alignment"),
            (ALIGNMENT * 2, "exactly 2x aligned"),
            (1, "single byte"),
            (10, "very small file"),
        ];
        ...
        for (idx, (size, description)) in test_cases.iter().enumerate() {
            ...
            let file_path = create_test_file_with_data(temp_dir.path(), &format!("test_{}.dat", idx), &data).unwrap();
            ...
            assert_eq!(&buf[..*size], &data[..], "Data mismatch for {}", description);
    ...
    fn test_empty_file() {
        let temp_dir = TempDir::new().unwrap();
        let file_manager = Arc::new(GlobalFileManager::new(ALIGNMENT));
        let file_path = create_test_file_with_data(temp_dir.path(), "empty.dat", &[]).unwrap();
        let mut reader = ManagedAlignedReader::new(file_manager, &file_path).unwrap();
    ...
    fn test_refill_buffer_at_eof() {
        ...
        let file_path = create_test_file_with_data(temp_dir.path(), "test_refill_eof.dat", &data).unwrap();
        let mut reader = ManagedAlignedReader::new(file_manager, &file_path).unwrap();
    ...
    fn test_seek_beyond_eof_then_read() {
        ...
        let file_path = create_test_file_with_data(temp_dir.path(), "test_seek_beyond.dat", &data).unwrap();
        let mut reader = ManagedAlignedReader::new(file_manager, &file_path).unwrap();
    ...
    fn test_position_after_partial_buffer_read() {
        ...
        let file_path = create_test_file_with_data(temp_dir.path(), "test_position.dat", &data).unwrap();
        let mut reader = ManagedAlignedReader::with_buffer_size(file_manager, &file_path, 4096).unwrap();
        ...
        reader.seek(50_000).unwrap();
        assert_eq!(reader.position(), 50_000);
        
        // Read and verify position updates correctly
        reader.read_exact(&mut buf).unwrap();
        assert_eq!(reader.position(), 50_100);
    }
    ...
    fn test_utf8_in_read_line() {
        ...
        let file_path = create_test_file_with_data(temp_dir.path(), "test_utf8.dat", content.as_bytes()).unwrap();
        let mut reader = ManagedAlignedReader::new(file_manager, &file_path).unwrap();
    ...
    fn test_with_start_position_edge_cases() {
        ...
        let file_path = create_test_file_with_data(temp_dir.path(), "test_start_pos.dat", &data).unwrap();
                
        // Test starting at various positions
        let test_positions = vec![0, 1, 511, 512, 513, 4095, 4096, 4097, 9999];
        
        for start_pos in test_positions {
            let mut reader = ManagedAlignedReader::with_start_position(
                Arc::clone(&file_manager),
                &file_path,
                start_pos,
                4096,
            ...
            assert_eq!(&buf[..bytes_read], &data[start_pos as usize..start_pos as usize + bytes_read]);
