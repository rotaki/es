use es::{GenSortInputDirect, SortInput, ExternalSorter, Sorter};
...
fn test_gensort_invalid_file_size() {
    ...
    assert!(result.unwrap_err().contains("not a multiple of record size"));
}
...
fn test_gensort_large_sort() {
    ...
    let input = GenSortInputDirect::new(&test_file).unwrap();
    let mut sorter = ExternalSorter::new_with_dir(
        2, 
        1024 * 1024,
        temp_dir.path()
    );
    ...
    for (key, payload) in output.iter() {
        if let Some(ref prev) = prev_key {
            assert!(key >= *prev, "Sort order violation: {:?} < {:?}", 
