use es::{
    ExternalSorter, Input, MergeStats, RunGenerationStats, RunsOutput, SortOutput, SortStats,
    TempDirInfo,
};
use std::sync::Arc;
...
fn main() -> Result<(), String> {
    ...
    println!(
        ...
        overall_stats.run_generation_time_ms
    );
...
fn print_run_gen_stats(stats: &RunGenerationStats) {
    ...
    for (i, run_info) in stats.runs_info.iter().enumerate() {
        println!(
            "  Run {}: {} entries, {} bytes",
            i, run_info.entries, run_info.file_size
        );
    }
    ...
    if let Some(ref io_stats) = stats.io_stats {
        ...
        println!(
            "  Read: {} ops, {} bytes",
            io_stats.read_ops, io_stats.read_bytes
        );
        println!(
            "  Write: {} ops, {} bytes",
            io_stats.write_ops, io_stats.write_bytes
        );
    }
}
